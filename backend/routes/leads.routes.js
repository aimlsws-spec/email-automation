...
router.get('/api/leads/pending', async (req, res) => {
  try {
    ...
    let query = `SELECT COUNT(*) AS count FROM leads WHERE user_id = ? AND LOWER(status) = 'pending'`;
    const params = [userId];
    if (campaignId) {
    ...
    console.error('❌ /api/leads/pending ERROR:', err.message);
  ...
  try {
    ...
    let query = `
      SELECT l.email AS id, l.name, l.email, l.company, l.status, l.follow_up_count,
        l.has_replied, l.last_activity_at, l.created_at, l.reply_detected_at,
        l.sender_email AS lead_sender_email, c.id AS campaign_id, c.name AS campaign_name,
        COALESCE(c.sender_email, l.sender_email) AS sender_email, c.subject
      FROM leads l
