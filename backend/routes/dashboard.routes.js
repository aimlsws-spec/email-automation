  ...
  try {
    ...
    const { rows: companies } = await pool.query(`
      SELECT
        ...
        ROUND(SUM(CASE WHEN reply_detected_at IS NOT NULL AND reply_detected_at != '' THEN 1 ELSE 0 END) * 100.0 / NULLIF(SUM(CASE WHEN status IN ('Sent','Follow-up 1','Follow-up 2','Replied') THEN 1 ELSE 0 END), 0), 1) AS reply_rate
      FROM leads
    ...
    const { rows: trend } = await pool.query(
      `SELECT company, DATE(last_sent_date) AS day, COUNT(*) AS sent_count
      FROM leads
      WHERE company IN (${placeholders}) AND user_id = ?
        AND last_sent_date IS NOT NULL AND last_sent_date != ''
    ...
    const campaigns = companies.map((c) => ({
      ...
      trend_delta: parseFloat(((parseFloat(c.reply_rate) || 0) - avgRate).toFixed(1)),
      chart_data: days.map((d) => trendMap[c.company][d] || 0),
    }));
    res.json({ campaigns });
  } catch (err) {
...
router.get('/api/dashboard/advanced-stats', async (req, res) => {
  try {
    ...
    const [leadsRows, logRows] = await Promise.all([
      pool.query(`SELECT SUM(CASE WHEN has_replied = 1 THEN 1 ELSE 0 END) AS converted_leads FROM leads WHERE user_id = ?`, [userId]),
      pool.query(`SELECT SUM(CASE WHEN status IN ('sent','success') THEN 1 ELSE 0 END) AS sent_count, COUNT(*) AS total_count FROM email_logs WHERE user_id = ?`, [userId]),
    ]);
    ...
    res.json({ reply_rate: replyRate, sent_rate: sentRate, converted_leads: convertedLeads });
  } catch (err) {
...
router.get('/api/dashboard/email-activity', async (req, res) => {
  ...
  try {
    ...
    const [sentRows, replyRows] = await Promise.all([
      pool.query(`SELECT ${fmtMap[trunc]} AS period, COUNT(*) AS sent_count FROM email_logs WHERE status IN ('success', 'sent') AND sent_at >= NOW() - INTERVAL ${mysqlInterval} AND user_id = ? GROUP BY 1 ORDER BY 1`, [userId]),
      pool.query(`SELECT ${replyFmtMap[trunc]} AS period, COUNT(*) AS reply_count FROM leads WHERE reply_detected_at IS NOT NULL AND reply_detected_at != '' AND reply_detected_at >= NOW() - INTERVAL ${mysqlInterval} AND user_id = ? GROUP BY 1 ORDER BY 1`, [userId]),
    ]);
    ...
    for (let i = countMap[trunc] - 1; i >= 0; i--) {
      ...
      else if (trunc === 'month') { d.setMonth(d.getMonth() - i); periods.push(d.toISOString().slice(0, 7)); }
      else { d.setFullYear(d.getFullYear() - i); periods.push(String(d.getFullYear())); }
    }
    const data = periods.map(period => ({ date: period, sent: sentMap[period] || 0, replies: replyMap[period] || 0 }));
    res.json({ data });
  } catch (err) {
...
router.get('/api/dashboard/stats', async (req, res) => {
  try {
    ...
    const [leadsResult, logsResult, campaignsResult] = await Promise.all([
      pool.query(`SELECT COUNT(*) AS leads_imported, SUM(CASE WHEN has_replied = 1 THEN 1 ELSE 0 END) AS replies_conversions FROM leads WHERE user_id = ?`, [userId]),
      pool.query(`SELECT SUM(CASE WHEN status IN ('sent', 'success') THEN 1 ELSE 0 END) AS emails_sent FROM email_logs WHERE user_id = ?`, [userId]),
      pool.query(`SELECT COUNT(*) AS active_campaigns FROM campaigns WHERE LOWER(status) = 'running' AND user_id = ?`, [userId]),
    ]);
    const r = leadsResult.rows[0];
...
router.get('/api/dashboard/recent-activity', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(`
      SELECT name AS lead_name, email AS lead_email, reply_detected_at, follow_up_count, last_sent_date
      FROM leads
      WHERE last_sent_date IS NOT NULL AND last_sent_date != '' AND user_id = ?
      ORDER BY last_sent_date DESC LIMIT 10
    `, [userId]);
    const events = rows.map((r) => ({
      lead_name: r.lead_name,
      lead_email: r.lead_email,
      action: r.reply_detected_at && r.reply_detected_at !== '' ? 'replied' : (parseInt(r.follow_up_count, 10) || 0) > 0 ? 'followup' : 'sent',
...
router.get('/api/dashboard/summary', async (req, res) => {
  try {
    ...
    const [statsResult, statusResult, activityResult, topCampaignResult, todayLogsResult, clicksResult] = await Promise.all([
      pool.query(`
        SELECT
          (SELECT COUNT(*) FROM email_logs WHERE status IN ('sent','success') AND DATE(sent_at) = CURDATE() AND user_id = ?) AS emails_sent_today,
          (SELECT COUNT(*) FROM campaigns WHERE LOWER(status) = 'running' AND user_id = ?) AS active_campaigns,
      ...
      pool.query(`SELECT status, COUNT(*) AS count FROM leads WHERE user_id = ? GROUP BY status`, [userId]),
      pool.query(`
        SELECT COALESCE(lead_email, email) AS email, subject,
          CASE WHEN status = 'failed' THEN 'failed' WHEN type LIKE 'follow_up_%' THEN 'followup' ELSE 'sent' END AS type,
          COALESCE(sent_at, created_at) AS timestamp
        FROM email_logs WHERE user_id = ?
        ORDER BY COALESCE(sent_at, created_at) DESC LIMIT 15
      `, [userId]),
      pool.query(`
        SELECT c.id, c.name, COUNT(l.email) AS sent,
          SUM(CASE WHEN l.has_replied = 1 THEN 1 ELSE 0 END) AS replies,
          ROUND(SUM(CASE WHEN l.has_replied = 1 THEN 1 ELSE 0 END) * 100.0 / NULLIF(COUNT(l.email), 0), 1) AS reply_rate
        FROM campaigns c LEFT JOIN leads l ON l.campaign_id = c.id
        WHERE c.user_id = ?
        GROUP BY c.id, c.name ORDER BY replies DESC, sent DESC LIMIT 1
      `, [userId]),
      pool.query(`
        SELECT SUM(CASE WHEN status IN ('sent','success') THEN 1 ELSE 0 END) AS sent_today,
        ...
        FROM email_logs WHERE DATE(sent_at) = CURRENT_DATE AND user_id = ?
      `, [userId]),
      pool.query(`
        SELECT (SELECT COUNT(*) FROM link_clicks WHERE DATE(clicked_at) = CURDATE() AND user_id = ?) AS clicks_today,
...
router.get('/api/dashboard/automation', DashboardController.getAutomation);
