...
async function uploadLeads(req, res) {
  ...
  try {
    ...
    let rows;
    try {
      rows = ext === '.csv' ? await parseCSV(req.file.buffer) : parseExcel(req.file.buffer);
    } catch (parseErr) {
    ...
    if (valid.length === 0) return res.status(400).json({ error: 'No valid leads found in the uploaded file.' });
    ...
    if (requestedCampaignId) {
      const { rows: byId } = await client.query(`SELECT id FROM campaigns WHERE id = ? AND user_id = ?`, [parseInt(requestedCampaignId, 10), userId]);
      if (byId.length > 0) {
        campaignId = byId[0].id;
        await client.query(`UPDATE campaigns SET status = 'Running', sender_email = COALESCE(?, sender_email), subject = ? WHERE id = ?`, [finalSenderEmail, subject, campaignId]);
      }
    }

    if (!campaignId) {
      const existingCampaign = await client.query(`SELECT id FROM campaigns WHERE name = ? AND user_id = ? ORDER BY id DESC LIMIT 1`, [campaignName, userId]);
      if (existingCampaign.rows.length > 0) {
        campaignId = existingCampaign.rows[0].id;
        await client.query(`UPDATE campaigns SET status = 'Running', sender_email = COALESCE(?, sender_email), subject = ? WHERE id = ?`, [finalSenderEmail, subject, campaignId]);
      } else {
        await client.query(`INSERT INTO campaigns (name, created_at, status, sender_email, subject, user_id) VALUES (?, NOW(), 'Running', ?, ?, ?)`, [campaignName, finalSenderEmail, subject, userId]);
        const { rows: lastIdRows } = await client.query(`SELECT LAST_INSERT_ID() AS insertId`);
        campaignId = lastIdRows[0]?.insertId;
        if (!campaignId) {
          const { rows: newCamp } = await client.query(`SELECT id FROM campaigns WHERE name = ? AND user_id = ? ORDER BY id DESC LIMIT 1`, [campaignName, userId]);
    ...
    for (const lead of batch) {
      await client.query(
        `INSERT INTO leads (email, name, company, campaign_id, status, last_activity_at, created_at, follow_up_count, message_id, initial_message_id, last_subject, inquiry_id, reply_detected_at, user_id)
         VALUES (?, ?, ?, ?, 'Pending', NOW(), NOW(), 0, '', '', '', '', '', ?)
         ON DUPLICATE KEY UPDATE
           name = VALUES(name), company = VALUES(company), campaign_id = VALUES(campaign_id),
           status = 'Pending', last_activity_at = NOW(), follow_up_count = 0,
           message_id = '', initial_message_id = '', last_subject = '', reply_detected_at = '',
           user_id = VALUES(user_id)`,
        [lead.email, lead.name || '', lead.company || '', campaignId, userId]
      );
      inserted++;
    }
    ...
    res.json({
      total: rows.length, valid: valid.length, inserted, skipped: skipped.length, campaignId,
...
async function sendEmails(req, res) {
  try {
    ...
    if (leads.length === 0) return res.json({ success: true, sent: 0, message: 'No valid rows found.' });
    let sent = 0;
    try {
      for (const lead of leads) {
        try {
          ...
          await pool.query(`INSERT INTO email_queue (lead_email, subject, html_body, status, campaign_id, user_id) VALUES (?, ?, ?, 'pending', ?, ?)`, [lead.email, subject, html, req.body.campaignId || lead.campaign_id || null, req.user.id]);
          sent++;
        } catch (err) {
    ...
    return res.json({ success: true, sent, message: `${sent} email(s) added to queue for processing.` });
  } catch (err) {
...
async function sendInitialEmail(req, res) {
  try {
    ...
    await pool.query(
      `INSERT INTO leads (email, name, company, status, email_provider, user_id)
       VALUES (?, ?, ?, 'Pending', ?, ?)
       ON DUPLICATE KEY UPDATE
         name = VALUES(name), company = VALUES(company),
         status = CASE WHEN status = 'Replied' THEN status ELSE 'Pending' END,
         email_provider = VALUES(email_provider), user_id = VALUES(user_id)`,
      [email, name, company || '', 'gmail', userId]
    );
